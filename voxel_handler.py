...
class VoxelHandler:
    def __init__(self, world):
        ...
        # resultado de lançamento de raios
        self.chunk = None
        self.voxel_id = None
        ...
        self.interaction_mode = 0 # 0: quebrar voxel, 1: colocar voxel
        self.new_voxel_id = 1

    def add_voxel(self):
        if self.voxel_id:
            # verificar id do voxel ao longo do normal
            result = self.get_voxel_id(self.voxel_world_pos + self.voxel_normal)

            # Tem algum lugar novo vazio?
            if not result[0]:
